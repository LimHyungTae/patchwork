...
ros::Publisher LabeledCloudPublisher;
...
void callbackNode(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  std::cout << "# cloud: " << pc_curr.size() << " -> " << pc_ground.size() << "\033[0m"
            << std::endl;
  ...
  //    sc_output << msg->header.seq << "," << time_taken << "," << precision <<
  //    "," << recall << "," << precision_naive << "," << recall_naive;
  //
  //    sc_output << std::endl;
  //    sc_output.close();
  ...
  if (is_kitti) {
    ...
    if (save_flag) {
      ...
      double accuracy;
      save_all_accuracy(
          pc_curr, pc_ground, acc_filename, accuracy, pc_curr_gt_counts, g_est_gt_counts);
  ...
  if (is_kitti) {
    ...
    if (GroundPublisher.getNumSubscribers()) GroundPublisher.publish(cloud2msg(pc_ground));
...
int main(int argc, char **argv) {
  ...
  LabeledCloudPublisher =
      nh.advertise<sensor_msgs::PointCloud2>("/benchmark/labeled_cloud", 100, true);
  ...
  ros::Subscriber NodeSubscriber =
      nh.subscribe<sensor_msgs::PointCloud2>("/patchwork/cloud", 5000, callbackNode);

  if (is_kitti) {
    std::cout << "\033[1;33mKITTI mode is activated. If not, please check "
                 "`is_kitti` parameter\033[0m"
              << std::endl;
  }
  ros::spin();
