...
bool condParam(ros::NodeHandle *nh,
    ...
    if (nh->hasParam(prefix + "/" + param_name)) {
        ...
        if (nh->getParam(ros::this_node::getName() + "/" + param_name, param_val)) {
            ROS_INFO_STREAM(
                "param '" << ros::this_node::getName() << "/" << param_name << "' -> '" << param_val << "'");
            return true;
        }
    }
...
class PatchWork {
 ...
 public:
    ...
    PatchWork(ros::NodeHandle *nh) {
        ...
        if (using_global_thr_) {
            cout << "\033[1;33m[Warning] Global elevation threshold is turned on :" << global_elevation_thr_
                 << "\033[0m" << endl;
 ...
 private:
    ...
    void perform_regionwise_ground_segmentation(Patch<PointT> &patch, const bool is_h_available = true);
    ...
    void extract_initial_seeds_(const pcl::PointCloud<PointT> &p_sorted, pcl::PointCloud<PointT> &init_seeds,
                                bool is_close_to_origin, bool is_h_available);
...
template<typename PointT>
inline
void PatchWork<PointT>::initialize(RegionwisePatches &patches, const ConcentricZoneModel &zone_model) {
    patches.clear();
    ...
    // In polar coordinates, `num_columns` are `num_sectors` and `num_rows` are `num_rings`, respectively
...
template<typename PointT>
inline
void PatchWork<PointT>::flush_patches(RegionwisePatches &patches) {
...
template<typename PointT>
inline
void PatchWork<PointT>::extract_initial_seeds_(const pcl::PointCloud<PointT> &p_sorted,
...
void PatchWork<PointT>::estimate_sensor_height(pcl::PointCloud<PointT> cloud_in) {
    ...
    for (int       i = 0; i < num_sectors_for_ATAT_; ++i) {
        if ((int) ring_for_ATAT[i].cloud_.size() < num_min_pts_) { continue; }
    ...
    cout
        << "\033[1;33m[ATAT] The sensor height is auto-calibrated via the ground points in the vicinity of the vehicle\033[0m"
        << endl;
...
void PatchWork<PointT>::estimate_ground(
    const pcl::PointCloud<PointT> &cloud_in, pcl::PointCloud<PointT> &ground,
    pcl::PointCloud<PointT> &nonground, double &time_taken) {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, num_patches), [&](tbb::blocked_range<int> r) {
        for (int k = r.begin(); k < r.end(); ++k) {
            ...
            if ((int) patch.cloud_.points.size() > num_min_pts_) {
                ...
                patch.status_ = determine_ground_likelihood_estimation_status(ring_idx,
                                                                              ground_z_vec,
                                                                              ground_z_elevation,
                                                                              surface_variable);
            } else {
    ...
    std::for_each(patch_indices_.begin(), patch_indices_.end(), [&](const std::pair<int, int> &index_pair) {
        ...
        if (status == FEW_POINTS) {
            ...
            if (verbose_) {
                std::cout << "\033[1;34m[Elevation] Rejection operated. Check "
                          << ring_idx
                          << "th param. of elevation_thr_: " << -sensor_height_ + elevation_thr_[ring_idx]
                          << " < " << feat.mean_(2) << "\033[0m\n";
...
template<typename PointT>
inline
void PatchWork<PointT>::pc2regionwise_patches(const pcl::PointCloud<PointT> &src, RegionwisePatches &patches) {
...
template<typename PointT>
inline
void PatchWork<PointT>::perform_regionwise_ground_segmentation(Patch<PointT> &patch, const bool is_h_available) {
    // 0. Initialization
...
template<typename PointT>
inline
geometry_msgs::PolygonStamped PatchWork<PointT>::set_polygons(int ring_idx, int sector_idx, int num_split) {
